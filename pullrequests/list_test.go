...
func (s *S) SetUpSuite(c *check.C) {
	...
	*s.stashInfo.Credential().User() = "test"
	...
	go func() {
		err := bbmock.RunServer(7993)
		if err != nil {
			c.Assert(err, check.IsNil)
		}
	}()

	for err := fmt.Errorf(""); err != nil; _, err = http.Get("http://localhost:7993/") {
...
func (s *S) TestListValidHost(c *check.C) {
	*s.host = "http://localhost:7993"
	...
	want, err := ioutil.ReadFile("mocked_responses/pullrequests_list.output")
	c.Assert(err, check.IsNil)
